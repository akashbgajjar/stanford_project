...
# "AvgSat"         "GradRate"       "Average_Cost"   "LowIncome"     
# "FirstGen"       "UGDS"           "UGDS_WHITE"     "UGDS_ASIAN"    
# "UGDS_AIAN"      "UGDS_NHPI"      "SAT_AVG"        "TUITIONFEE_IN" 
# "TUITIONFEE_OUT" "C150_4_WHITE"   "C150_4_BLACK"   "C150_4_HISP"   
# "C150_4_ASIAN"   "PPTUG_EF"       "AVGFACSAL"     
# RMSE = 0.03351798
# Final Check
training=trainData[,selectedFeatures]
testing=testData[,selectedFeatures]
...
#RMSE 0.03351753477
modelfinalENET=train(GradRate ~.,training,method="enet")
predictfinalENET=predict(modelfinalENET,testing)
ENETfinalRMSE=postResample(predictfinalENET,testing$GradRate)[1]
ENETfinalRMSE


#Repeated K-fold cross validation with the training --19 Variables--

train_control <- trainControl(method="repeatedcv", number=5, repeats=3)
mdlLM <-train(GradRate~., data=training,trControl=train_control, method="lm")
mdlRF <-train(GradRate~., data=training,trControl=train_control, method="rf")
mdlSVM3 <-train(GradRate~., data=training,trControl=train_control, method="svmLinear3")
mdlSVM2 <-train(GradRate~., data=training,trControl=train_control, method="svmLinear2")
mdlPCR <-train(GradRate~., data=training,trControl=train_control, method="pcr")
mdlSVMP <-train(GradRate~., data=training,trControl=train_control, method="svmPoly")
mdlBR <-train(GradRate~., data=training,trControl=train_control, method="bridge")
mdlBRNN <-train(GradRate~., data=training,trControl=train_control, method="brnn")
mdlNN <-train(GradRate~., data=training,trControl=train_control, method="neuralnet")
mdlENET <-train(GradRate~., data=training,trControl=train_control, method="enet")

allModels=resamples(list(LinearRegression=mdlLM,RandomForest=mdlRF,SupporVector3=mdlSVM3,
                         SupporVector2=mdlSVM2,PrincipalComponent=mdlPCR,
                         SupportVectorPoly=mdlSVMP,
                         BayesianRidge=mdlBR,BayesianRegularNN=mdlBRNN,NeuralNetwork=mdlNN,
                         Elasticnet=mdlENET))
